...
public class Rule6Handler extends BaseRuleHandler{
	...
	public Rule6Handler(Rule rule,RuleTransactionTypeWithinDays params) {
	...
	protected void detect() {
		...
		concernedList = ruleRepo.getTellerAmountOver(params.getTransactionTypes(), params.getTransactionAmount(), date);
	...
	private boolean hasChangeStatus(TellerAccountResult fraud,Date date) {
		...
		int changeStatusCount = BeanHolder.getInstance().getDtjRepo().hasChangeStatusToActive(fraud.getAccount_number(), date);
		
		return changeStatusCount > 0;
