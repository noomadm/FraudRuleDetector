...
public class Rule5Handler extends BaseRuleHandler{
	...
	public Rule5Handler(Rule rule,RuleTransactionTypeWithinDays params) {
	...
	protected void detect() {
		...
		concernedList = ruleRepo.getTellerCancelTransaction(params.getTransactionTypes(), params.getTransactionAmount(), date);
	...
	private boolean isFraud(List<DTJ> transactions) {
		...
		for(DTJ tran : transactions) {
			...
			if(tran.getTrans_amount() <= -1*params.getTransactionAmount()) {
				cancelSequenceNo = tran.getTrans_sequence_no();
				continue;
			}
			
			boolean isExpectTran = cancelSequenceNo+1 == tran.getTrans_sequence_no();
